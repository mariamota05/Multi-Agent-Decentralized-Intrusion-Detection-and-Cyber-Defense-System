...
def _log(agent_type: str, jid: str, msg: str) -> None:
    ...
    Args:
        agent_type (str): The type of the agent (e.g., "MonitoringAgent").
...
class MonitoringAgent(Agent):
    """Agent that monitors network messages and generates alerts using CNP."""

    class ResourceBehaviour(PeriodicBehaviour):
        """Periodically updates monitor resource metrics."""
        async def run(self):
            """Update CPU and bandwidth usage based on analyzed messages and active auctions.

            Calculates CPU and bandwidth based on:
            1. Base load (20% CPU, 10% BW).
            2. Analysis load (1.5% CPU, 1.0% BW per message analyzed since last cycle).
            3. Auction load (10% CPU, 5% BW per active CNP auction).
            Resets message analysis counter after calculation.
            """
    ...
    class CNPInitiatorBehaviour(CyclicBehaviour):
        ...
        async def on_start(self):
            """Initialize CNP initiator behaviour and alerted_senders dict."""
            _log("MonitoringAgent", str(self.agent.jid), "CNP Initiator behaviour started")
            self.alerted_senders: Dict[str, float] = {}

        async def run(self):
            """Handle incoming CNP protocol messages and dispatch to handlers.

            Listens for PROPOSE (from bidders) and INFORM (from winner on task completion) messages.
            Timeout is 1 second per cycle.
            """
        ...
        async def handle_propose(self, msg: Message):
            """Process a PROPOSE message for a CNP auction.

            Appends the proposal (bidder JID and score) to the pending incident.
            If all expected proposals are received (based on the number of response agents),
            it triggers the evaluation process.

            Args:
                msg (Message): The PROPOSE message containing the incident ID and availability score.
            """
        ...
        async def evaluate_proposals(self, incident_id: str):
            """Evaluate all proposals for an incident and award contract to best bidder.

            Selects the bidder with the lowest 'availability_score' (lowest score wins).
            Sends ACCEPT_PROPOSAL to the winner, including incident details and intensity.
            Sends REJECT_PROPOSAL to all other bidders.

            Args:
                incident_id (str): The ID of the incident to evaluate.
            """
            pending = self.agent.get("pending_cfps") or {}
        ...
        async def handle_inform(self, msg: Message):
            """Handle INFORM message indicating incident completion.

            Removes the incident from the pending CNP list.

            Args:
                msg (Message): The INFORM message from the winning response agent.
            """
            incident_id = msg.get_metadata("incident_id")
    ...
    class MonitorBehav(CyclicBehaviour):
        ...
        def __init__(self, suspicious_window: int = 3, suspicious_threshold: int = 5, *args, **kwargs):
            """Initialize monitoring behaviour with detection windows and thresholds.

            The behaviour tracks two types of events:
            1. Rate-based events (DDoS detection): Messages per `suspicious_window` seconds.
            2. Keyword-based events (Insider/Malware): Rate of low-priority keywords over a 60-second window.

            Args:
                suspicious_window (int): Time window (seconds) for rate-based DDoS detection.
                suspicious_threshold (int): Message count threshold for DDoS detection within the window.
            """
        ...
        async def on_start(self):
            """Initialize alerted_senders dict when behaviour starts."""
            _log("MonitoringAgent", str(self.agent.jid), "Monitoring behaviour started")
            # Tracks senders recently alerted to prevent flooding the CNP mechanism
            self.alerted_senders: Dict[str, float] = {}

        async def initiate_cnp(self, offender_jid: str, threat_type: str, alert: Dict[str, Any], victim_jid: str = None):
            """Initiate a CNP auction for a detected incident.

            Generates a unique incident ID, stores the incident details in `pending_cfps`,
            and sends a CFP message to all configured response agents.

            Args:
                offender_jid (str): The JID of the sender flagged as malicious.
                threat_type (str): The classified type of the threat (e.g., 'ddos', 'malware').
                alert (Dict[str, Any]): Detailed dictionary containing the alert information.
                victim_jid (str, optional): The JID of the intended victim node. Defaults to None.
            """
        ...
        async def process_message(self, msg: Message):
            """Analyze a message for suspicious activity and trigger alerts if needed.

            Applies rate-based (DDoS) and keyword-based (Malware/Insider) heuristics.
            Uses a probabilistic model to simulate evasion by high-intensity attackers.
            If detected and not silenced, initiates the CNP auction via `initiate_cnp`.

            Args:
                msg (Message): The message object to analyze.
            """
            ...
            if suspicious:
                ...
                # Minimal silencing window: Allow rapid CNP generation to stress response agent
                # 0.5s cooldown just prevents duplicate detections from same message burst
                if sender not in self.alerted_senders:
                    self.alerted_senders[sender] = now + 0.5
                else:
                    return
        ...
        async def run(self):
            """Main loop to receive and process messages for monitoring and alerting.

            Processes two types of messages:
            1. Messages for heuristic analysis (`network-copy` or standard messages).
            2. `threat-alert` messages sent by router/node firewalls for self-detected threats.
            All other CNP-related protocols are ignored here, as they are handled by `CNPInitiatorBehaviour`.
            """
            msg = await self.receive(timeout=1)
            if msg:
                protocol = msg.get_metadata("protocol")
    ...
    async def setup(self):
        """Setup the monitoring agent, initialize metrics, and add behaviours.

        Initializes agent state (CPU, BW, CNP tracking) and adds:
        1. `ResourceBehaviour` (periodic metric update).
        2. `MonitorBehav` (cyclic message analysis and alerting).
        3. `CNPInitiatorBehaviour` (CNP auction handling), if response agents are configured.
        """
...
async def main():
    """Main entry point for running the MonitoringAgent from command line.

    Parses command line arguments for JID, password, target nodes, response agents,
    detection window/threshold, and auto-block option. Starts the agent and waits
    ...
    # Update detection parameters in the MonitorBehav instance if arguments were passed
...
if __name__ == "__main__":
    """Run the MonitoringAgent if executed as a script."""
    spade.run(main())